...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough SLCT to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your SLCT.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate SilverlineSwap Profile to take part in next IFO‘s!')
                : t('You need an active SilverlineSwap Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    if (ifo.version === 3 && getBalanceNumber(credit) === 0) {
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average SLCT balance available to commit in the IFO SLCT pool.')}
            </MessageText>{' '}
      ...
      return userPoolCharacteristics.amountTokenCommittedInLP.isEqualTo(0) ? (
        <Flex flexDirection="column" alignItems="center">
          ...
          <Text textAlign="center" fontSize="14px">
            {t('To participate in the next IFO, stake some SLCT in the IFO SLCT pool!')}
          </Text>
